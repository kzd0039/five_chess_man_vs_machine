...
def man_vs_machine_battle(algorithm_name):
    print('Current available algorithms:' + '\n' + '1:minimax  2:alpha_beta_pruning  3:heuristic   4:greedy')
    ...
    while (True):
        ...
        while True:
            position = input('please enter the row and column number of the chess (seperate by white space):')
            try:
                x, y = position.split(' ')
                x = int(x)
                y = int(y)
...
def availableAlgorithms():
    return {1: _minimax, 2: _alpha_beta_pruning, 3: _heuristic, 4: _greedy}
